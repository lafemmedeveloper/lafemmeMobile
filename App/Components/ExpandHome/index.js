...
export default (dta) => {
  ...
  const [animationItem, onChangeanimationItem] = React.useState(new Animated.Value(0));
  ...
  return (
    <Animated.View style={[styles.container]}>
      <TouchableOpacity
        onPress={() => {
          duration = 250;
          let toHeight = itemHeight * (products && products.length ? products.length : 50);
        ...
        {products &&
          ...
          products.map((product, index) => {
            return (
              <TouchableOpacity
                onPress={() => {
                  const result = _.filter(addOns, (o) => o.idProduct == product.id);
              ...
              >
                {index != 0 && (
                  <Animated.View
                    opacity={opacity}
                    style={{ width: '95%', height: 1, alignSelf: 'center', backgroundColor: 'rgba(0,0,0,0.05)' }}
                  ></Animated.View>
                )}
                <Animated.View
                  opacity={opacity}
                ...
                >
                  <View style={{ flex: 1, marginHorizontal: 10 }}>
                    ...
                    >
                      <Text style={Fonts.style.semiBold(Colors.dark, Fonts.size.medium, 'left')}>
                        {product.name.toUpperCase()}
                      </Text>
                    </View>
                    <View>
                      <Text style={Fonts.style.regular(Colors.gray, Fonts.size.medium, 'left')}>
